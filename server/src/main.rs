...
async fn ingest(State(state): State<Arc<AppState>>, Json(req): Json<IngestRequest>) -> impl IntoResponse {
    ...
    for item in req.logs {
        let id = Uuid::new_v4();
        let category = item.category.unwrap_or_else(|| "system".into());
        let category = classify_with_rules(&state.pool, &item.source_type, &item.source_name).await.unwrap_or(category);
...
async fn classify_with_rules(pool: &PgPool, source_type: &str, source_name: &str) -> Option<String> {
    let rules: Vec<LogRule> = sqlx::query_as("SELECT id, source_type, source_pattern, category, priority, enabled FROM log_rules WHERE enabled = TRUE ORDER BY priority DESC")
...
async fn delete_rule(State(state): State<Arc<AppState>>, axum::extract::Path(id): axum::extract::Path<Uuid>) -> impl IntoResponse { let result=sqlx::query("DELETE FROM log_rules WHERE id=$1").bind(id).execute(&state.pool).await; Json(serde_json::json!({"success":result.is_ok()})) }
...
#[tokio::main] async fn main()->anyhow::Result<()> { tracing_subscriber::fmt().with_env_filter(env::var("RUST_LOG").unwrap_or_else(|_|"info".into())).init(); let database_url=env::var("DATABASE_URL")?; let pool=PgPoolOptions::new().max_connections(20).connect(&database_url).await?; let(tx,_)=broadcast::channel(2048); let state=Arc::new(AppState{pool,tx}); let app=Router::new().route("/health",get(||async{"ok"})).route("/api/agent/logs",post(ingest)).route("/api/logs",get(logs)).route("/api/logs/categories",get(categories)).route("/api/logs/rules",get(rules).post(create_rule)).route("/api/logs/rules/:id",axum::routing::delete(delete_rule)).route("/api/logs/ws",get(ws_handler)).layer(CorsLayer::permissive()).layer(TraceLayer::new_for_http()).with_state(state); let addr:SocketAddr=env::var("LISTEN_ADDR").unwrap_or_else(|_|"0.0.0.0:8080".into()).parse()?; info!(%addr,"loghub server started"); let listener=tokio::net::TcpListener::bind(addr).await?; axum::serve(listener,app).await?; Ok(()) }
