...
async fn send_batch(client: &Client, config: &RuntimeConfig, batch: &[LogRecord]) {
    ...
    let mut request = client.post(format!("{}/api/agent/logs", config.server_url)).json(&serde_json::json!({"logs": batch}));
    if let Some(token)=&config.token { request=request.bearer_auth(token); }
    if let Err(e)=request.send().await { warn!(error=%e,"failed to send logs"); }
}
...
fn build_config(file: FileConfig)->RuntimeConfig {
    RuntimeConfig {
        ...
        systemd_units:file.systemd_units.unwrap_or_default(),
    }
}
...
async fn docker_collector(tx:mpsc::Sender<LogRecord>,host:String)->Result<()> {
    ...
    for c in docker.list_containers(None).await? {
        ...
        tokio::spawn(async move {while let Some(item)=stream.next().await {if let Ok(output)=item {let _=tx2.send(LogRecord{log_time:Utc::now(),host:host2.clone(),source_type:"docker".into(),source_name:name.clone(),category:None,level:None,message:output.to_string(),metadata:serde_json::json!({"container_id":id})}).await;}}});
    }
    Ok(())
}
...
async fn systemd_collector(tx:mpsc::Sender<LogRecord>,host:String,units:Vec<String>)->Result<()> {
    for unit in units {
        ...
        tokio::spawn(async move {while let Ok(Some(line))=lines.next_line().await {if let Ok(v)=serde_json::from_str::<Value>(&line){let _=tx2.send(LogRecord{log_time:Utc::now(),host:host2.clone(),source_type:"systemd".into(),source_name:unit.clone(),category:None,level:None,message:v.get("MESSAGE").and_then(Value::as_str).unwrap_or("").to_string(),metadata:v}).await;}}});
    }
    Ok(())
}
...
async fn main()->Result<()> {
    ...
    let agent_id=identity::load_or_create(None)?;
    let heartbeat_config=config.clone();
    tokio::spawn(async move {let _=heartbeat::run_heartbeat(heartbeat_config.server_url,heartbeat_config.token,agent_id,heartbeat_config.host).await;});

    let (tx,mut rx)=mpsc::channel::<LogRecord>(10000);
    let client=Arc::new(Client::new());
    let sender_config=config.clone(); let sender_client=client.clone();
